...
- P1（全新 Binary V1 底座代码已收口，生产协商关闭）：旧实验实现从未被 Client/Gateway/Server/Realtime 引用，reader/writer、旧 format、codec facade 与兼容窗口已全部删除。首个候选格式为 `chatapp-bin-v1`；10-byte 帧头、JSON 默认路径和生产能力位均不变。
  ...
  - **消费者接入验证（2026-08-12）：** Client（`Chat_App`）与 Gateway（`ChatAppTCP_Server`）已仅升级 `ChatApp.Protocol.Tcp` / `ChatApp.Protocol.Tcp.Json` 到 0.5.0，并在各自 `packages/` 离线 feed 与 lock file 下通过 locked restore、Release build。Client 全套测试 `283/283` 通过；Gateway 测试串行执行 `566/566` 通过，Redis 互操作 1 项因未设置 `CHATAPP_TEST_REDIS` 跳过。Gateway 默认并行执行稳定只有既有的 `DirectSocket + PersistentSendLoop` 参数化用例失败，单独执行该方法 `6/6` 通过，暂按测试并发隔离问题记录，不修改生产协议代码。两端均未引用 `ChatApp.Protocol.Tcp.Binary`、`ChatApp.Binary.Core` 或 generator，握手、Resume 和 `ServerHello.PayloadFormat` 仍为 JSON；生产 binary capability 继续关闭。
  - **发布状态：** 本候选不可变提交已在 detached clean checkout 完成 locked restore、Release build/test、两次 clean pack+normalize 和七包 SHA-256 记录；0.5.0 JSON 包已在两个消费者的本地离线 feed 完成短时隔离消费验证。仍需审核后发布内部 feed，脏工作树包不发布，且包发布不自动打开生产协商。详细规则见 [`BINARY-PROTOCOL.md`](BINARY-PROTOCOL.md)。
