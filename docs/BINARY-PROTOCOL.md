...
> 收口状态（2026-08-19）：当前已展开批次完成测试级收口，生产寄存器以控制命令为主并已纳入首个 History response（见下方覆盖与未覆盖清单）。已展开 schema 落地位置与覆盖：
> - **生产 Schemas 程序集 + `TcpBinaryWireCodec` 寄存器（7 个命令）**：`ClientHello`/`ServerHello`/`GoAway`/`ResumeResponse`/`ProtocolErrorFrame`/`MessageHistoryRequest`/`MessageHistoryPage`（`Error`=500 复用错误帧；`MessageHistoryPage` 复用 `MessageHistoryResponse` schema，含其嵌套 `MessageHistoryItem`/`MessageHistoryCursor`/`TcpAttachmentRef`/`MessageReactionSummary` 生产 schema）；未覆盖命令与畸形/超限 payload 一律 fail-closed（`SchemaNotCovered`/`DecodeFailure`），不构成半套运行入口。
...
> - Shared 全量回归全绿：Binary.Core 37 / EncoderOnly 1 / Protocol.Tcp.Binary 39 / Generator 15 / Architecture 110，`dotnet build -c Release` 0 警告 0 错误。
>
> **覆盖统计**（以 `PacketCommand` 枚举为准）：总命令值 **85** = 寄存器已覆盖 **7**（`ClientHello`/`ServerHello`/`GoAway`/`ResumeResponse`/`MessageHistoryRequest`/`MessageHistoryPage`/`Error`→`ProtocolErrorFrame`）+ 未覆盖 **78**。寄存器 fail-closed，未覆盖命令不构成任何可运行入口。
>
> **未覆盖命令明确清单**（78 个，无 schema，寄存器 fail-closed，按功能分组）：认证/握手续传 **3** `AuthenticationRequest`/`AuthenticationResponse`/`ResumeRequest`；心跳 **2** `Heartbeat`/`HeartbeatAcknowledgement`；消息正文 **17** `ChatMessage`/`MessageAcknowledgement`/`MessageReceipt`/`MessageReceiptAcknowledgement`/`MessageReceiptUpdated`/`MessageEditRequest`/`MessageEditAck`/`MessageEdited`/`MessageRecallRequest`/`MessageRecallAck`/`MessageRecalled`/`AddReactionRequest`/`AddReactionAck`/`ReactionAdded`/`RemoveReactionRequest`/`RemoveReactionAck`/`ReactionRemoved`；历史/会话 **11** `ConversationListRequest`/`ConversationListPage`/`ConversationMarkReadRequest`/`ConversationMarkReadResponse`/`ConversationChanged`/`UnreadCountChanged`/`SyncBootstrapRequest`/`SyncBootstrapResponse`/`ConversationSetPrefsRequest`/`ConversationSetPrefsResponse`/`ConversationRead`；输入状态 **2** `TypingNotify`/`TypingUpdate`；在线 **4** `PresenceQuery`/`PresenceSnapshot`/`PresenceChanged`/`PresenceUnwatch`；群组 **20** `CreateGroupRequest`/`CreateGroupResponse`/`AddGroupMembersRequest`/`AddGroupMembersResponse`/`RemoveGroupMemberRequest`/`RemoveGroupMemberResponse`/`LeaveGroupRequest`/`LeaveGroupResponse`/`ChangeMemberRoleRequest`/`ChangeMemberRoleResponse`/`ListGroupMembersRequest`/`ListGroupMembersResponse`/`MemberJoined`/`MemberLeft`/`MemberRemoved`/`RoleChanged`/`MembersAddedUpdate`/`ConversationDissolvedUpdate`/`DissolveGroupRequest`/`DissolveGroupResponse`；关系 **5** `RelationshipListChanged`/`RelationshipCommandRequest`/`RelationshipCommandResponse`/`RelationshipListRequest`/`RelationshipListResponse`；附件 **5** `AttachmentLifecycleChanged`/`AttachmentFinalizeRequest`/`AttachmentFinalizeResponse`/`AttachmentDownloadAuthorizeRequest`/`AttachmentDownloadAuthorizeResponse`；推送 **4** `RegisterPushTokenRequest`/`RegisterPushTokenResponse`/`UnregisterPushTokenRequest`/`UnregisterPushTokenResponse`；已读回执 **2** `MessageReadReceiptQueryRequest`/`MessageReadReceiptQueryResponse`；通话 **3** `CallCommandRequest`/`CallCommandResponse`/`CallSignal`。这些命令按关系/语音消息/通话的真实需要分批补齐，不阻塞当前 JSON 主链路。
...
> **门控现状（2026-08-19）**：BIN-INTEGRATION-3 被一组硬条件门控，任一未满足则**不得**在 Client/Gateway 切换连接级格式。逐项状态如下：
> - **[未满足] 实际运行命令目录完整**：连接级 binary 必须在完整握手后固定一种格式，而当前寄存器仅覆盖 7/85 命令值，主链路实际用到的 `ChatMessage`、`Heartbeat`、关系/通话等大量命令仍无 schema。帧头没有逐帧格式位，因此残缺目录下无法安全启用（见 BIN-SCHEMA-2 收口第 71 行约束）。
> - **[已满足] 主链路前置**：关系（REL-E2E）、语音消息（VOICE-MSG）、通话（CALL-E2E）主链路均已关闭，故此支撑项的主链路前置不再阻塞（见 NEXT-STAGE）。
> - **[未满足] 收益与稳定性证据**：尚缺 80/320/640 msg/s 的 5–20 分钟短测；必须证明 payload 体积或 codec CPU/分配有**稳定收益**、**零漏投/重复**且 **p99 无不可解释回退**，才允许改变 JSON 默认路径。
> - **[未满足] 双端实现与验收**：Client/Gateway 双 codec 接入、协商后连接级固定格式、fanout 按格式分组编码一次并共享、JSON fallback、GoAway/重连、混合格式 fanout 与短测验收均未开始。
>
> 综上：当前只保留底座与离线验证能力；在命令目录与收益证据达标前，连接级格式保持 JSON，不推进双 codec 切换。
...
- [x] 新增 `TcpBinaryWireCodecTests`（8 项）：协商识别、控制/历史/错误帧往返、HistoryPage 往返、分段解码、未覆盖 fail-closed、畸形 fail-closed、超限 fail-closed。Shared 全量回归：Binary.Core 37 / EncoderOnly 1 / Protocol.Tcp.Binary 39 / Generator 15 / Architecture 110 全部通过，`dotnet build ChatApp.Shared.slnx -c Release` 0 警告 0 错误。架构边界补充：契约包允许 `OutputItemType="Analyzer"` 的 ProjectReference 指向 generator。
