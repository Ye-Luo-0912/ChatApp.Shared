...
>
> **未覆盖命令明确清单**（无 schema，寄存器 fail-closed，按功能分组）：认证/握手续传 `AuthenticationRequest`/`AuthenticationResponse`/`ResumeRequest`；心跳 `Heartbeat`/`HeartbeatAcknowledgement`；消息正文 `ChatMessage`/`MessageAcknowledgement`/`MessageReceipt`/`MessageReceiptAcknowledgement`/`MessageReceiptUpdated`/`MessageEditRequest`/`MessageEditAck`/`MessageEdited`/`MessageRecallRequest`/`MessageRecallAck`/`MessageRecalled`/`AddReactionRequest`/`AddReactionAck`/`ReactionAdded`/`RemoveReactionRequest`/`RemoveReactionAck`/`ReactionRemoved`；历史/会话 `ConversationListRequest`/`ConversationListPage`/`ConversationMarkReadRequest`/`ConversationMarkReadResponse`/`ConversationChanged`/`UnreadCountChanged`/`SyncBootstrapRequest`/`SyncBootstrapResponse`/`ConversationSetPrefsRequest`/`ConversationSetPrefsResponse`/`ConversationRead`；输入状态 `TypingNotify`/`TypingUpdate`；在线 `PresenceQuery`/`PresenceSnapshot`/`PresenceChanged`/`PresenceUnwatch`；群组 `CreateGroupRequest`/`CreateGroupResponse`/`AddGroupMembersRequest`/`AddGroupMembersResponse`/`RemoveGroupMemberRequest`/`RemoveGroupMemberResponse`/`LeaveGroupRequest`/`LeaveGroupResponse`/`ChangeMemberRoleRequest`/`ChangeMemberRoleResponse`/`ListGroupMembersRequest`/`ListGroupMembersResponse`/`MemberJoined`/`MemberLeft`/`MemberRemoved`/`RoleChanged`/`MembersAddedUpdate`/`ConversationDissolvedUpdate`/`DissolveGroupRequest`/`DissolveGroupResponse`；关系 `RelationshipListChanged`/`RelationshipCommandRequest`/`RelationshipCommandResponse`/`RelationshipListRequest`/`RelationshipListResponse`；附件 `AttachmentLifecycleChanged`/`AttachmentFinalizeRequest`/`AttachmentFinalizeResponse`/`AttachmentDownloadAuthorizeRequest`/`AttachmentDownloadAuthorizeResponse`；推送 `RegisterPushTokenRequest`/`RegisterPushTokenResponse`/`UnregisterPushTokenRequest`/`UnregisterPushTokenResponse`；已读回执 `MessageReadReceiptQueryRequest`/`MessageReadReceiptQueryResponse`；通话 `CallCommandRequest`/`CallCommandResponse`/`CallSignal`。这些命令按关系/语音消息/通话的真实需要分批补齐，不阻塞当前 JSON 主链路。
